...
parent_directory = 'Flock-'+str(folder_num)
os.mkdir(parent_directory)
folder = parent_directory+"/data"
os.mkdir(folder)
info_file = open(parent_directory+"/info.txt", "w")
np.save("folder_num", np.array([folder_num+1]))
...
class Flock:
    def __init__(self, N=2, initial_conditions="normal", dimension=2, model=Vicsek(1), total_steps=100, step_size=0.001, measure=True, animate=False):  # N = number of particles
    ...
    def time_step(self):  # moving one step forward in time
        ...
        # storing data
        self.data[self.step] = [self.continuous_t, self.Gamma, self.Lambda, self.K, self.E, *self.P, *self.R_cm, *self.V_cm]
        print(self.data[self.step])
        # time step
...
flock = Flock(N=10, total_steps=100, step_size=0.001, dimension=2, model=Smale(beta=0.3, K=1, sigma=1))

# try:
# (N, initial_conditions="normal", dimension=2,
# model=Vicsek(1), total_steps=100, step_size=0.001, measure=False, animate=False)
# flock = Flock(2, total_steps=1000)
# except:
#    Flag = True
#    Box.restart()
